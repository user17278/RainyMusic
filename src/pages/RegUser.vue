<template>
    <div class="reg-user-page-mask" v-on:click.self="clickOtherAreaToClose">
        <div class="reg-user-page">
            ...
            <div class="user-name">
                <button class="reg-user-next" ref="loginNext"><i class="fa fa-angle-right"
                        aria-hidden="true"></i></button>
                <input type="text" ref="userIdInput" placeholder="请输入1-10位的用户名 " v-model="userName"
                    v-on:keyup.enter="idCheck(userName)">
                <router-view></router-view>
            </div>
            <div class="user-tips">{{  checkTip  }}</div>
            <button class="reg-btn" ref="regBtn" v-on:click="regUser" v-show="userPassword">注 册 <i
                    class="fa fa-angle-double-right" aria-hidden="true"></i></button>
            <div class="reg-tips">
                <div class="separator"></div>
                <div class="privacy-img"><img src="../img/privacy-icon.png" alt=""></div>
                <div class="privacy-tips">您的 RainyMusic ID 信息被用于确保您能够安全登录并访问您的数据。<a href="">了解数据的管理方式。</a></div>
                <div>我已有 RainyMusic ID。
                    <router-link to="/login">马上登录
...
export default {
    ...
    methods: {
        ...
        idCheck: function (input) {
            this.$axios.post('http://127.0.0.1:3000/user/regUserNameCheck',
                {
                    username: input,
                }).then((res) => {
        ...
        regUser() {
            this.$axios.post('http://127.0.0.1:3000/user/userReg', {
